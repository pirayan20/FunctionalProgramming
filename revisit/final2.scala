...
object final2 {
  ...
  def helper(l: List[String])(f1: String => String)(f2: String => Boolean)(acc: List[String]): List[String] = {
    if (l.isEmpty) return acc
    if (f2(l.head)) helper(l.tail)(f1)(f2)(acc ++ List(f1(l.head)))
    else helper(l.tail)(f1)(f2)(acc ++ List(l.head))
  }
